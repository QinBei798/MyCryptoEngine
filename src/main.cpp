...
void print_menu() {
  ...
  std::cout << "  [4]  Exit\n";
  std::cout << "  [5]  Encrypt File — secure storage to .sm4z container\n";
  std::cout << "  [6]  Decrypt File — extract from .sm4z container\n";
  thin_line();
...
void do_decrypt(const SM4AVX2 &engine, const std::vector<uint8_t> &iv) {
  std::cout << "\n  Enter hex ciphertext (spaces/colons/0x ignored, empty line "
               "= cancel):\n"
...
void do_benchmark(const std::vector<uint8_t> &key,
  ...
  std::cout << "\n  Enter data size in MB (e.g. 10, 100, 1000, empty line = "
               "cancel):\n  > "
            << std::flush;
  ...
  double throughput_std =
      (ms_standard > 0) ? (static_cast<double>(size_mb) * 1000.0 / ms_standard)
                        : 0.0;
  double throughput_avx =
      (ms_avx2 > 0) ? (static_cast<double>(size_mb) * 1000.0 / ms_avx2) : 0.0;
  double speedup =
      (ms_avx2 > 0) ? static_cast<double>(ms_standard) / ms_avx2 : 0.0;

  std::cout << "\n";
  std::cout << "  ╔══════════════════════════════════════════════════════╗\n";
  std::cout << "  ║        " << std::setw(4) << size_mb
            << " MB  ENCRYPTION  RESULTS"
            << "                     ║\n";
  std::cout << "  ╠════════════════════╤═════════════╤══════════════════╣\n";
  ...
  std::cout << "  ║  SM4Standard       │  " << std::setw(6) << ms_standard
            << " ms   │  " << std::fixed << std::setprecision(1) << std::setw(7)
            << throughput_std << " MB/s     ║\n";
  ...
  std::cout << "  ╠════════════════════╧═════════════╧══════════════════╣\n";
  std::cout << "  ║  AVX2 Acceleration Ratio:  " << std::fixed
  ...
  std::cout << "\n  ";
  if (match) {
    std::cout << "[✓] SUCCESS: AVX2 ciphertext byte-for-byte identical to "
                 "scalar baseline.\n";
  } else {
...
void do_encrypt_file() {
  ...
  double ratio = (original_size > 0) ? static_cast<double>(compressed.size()) /
                                           original_size * 100.0
                                     : 100.0;
  ...
  std::cout << "  ║  Wall Time    : " << std::left << std::setw(35)
            << (std::to_string(ms) + " ms") << "║\n";
  std::cout << "  ╚══════════════════════════════════════════════════════╝\n";
  std::cout << "  [✓] Container .sm4z: Salt + IV + OriginalSize + HMAC all "
               "embedded.\n";
}
...
void do_decrypt_file() {
  ...
  std::streamsize fsize = ifs.tellg();
  if (fsize < 76) {
    std::cout << "  [✗] Error: File too small to be a valid V3 .sm4z container "
                 "(< 76 bytes).\n";
    return;
  }
  ...
  if (file_bytes[0] != 'S' || file_bytes[1] != 'M' || file_bytes[2] != '4' ||
      file_bytes[3] != 'Z') {
    std::cout << "  [✗] Error: Invalid secure storage format (Magic Bytes "
                 "Mismatch)\n";
    std::cout << "          Expected: SM4Z  Got: " << file_bytes[0]
              << file_bytes[1] << file_bytes[2] << file_bytes[3] << "\n";
    return;
  }
  ...
  std::vector<uint8_t> file_iv(file_bytes.begin() + 20,
                               file_bytes.begin() + 36);
  ...
  std::vector<uint8_t> ciphertext(file_bytes.begin() + 44,
                                  file_bytes.begin() + 44 + ct_size);
  std::vector<uint8_t> stored_hmac(file_bytes.end() - 32, file_bytes.end());

  // Header + Ciphertext (everything except the footer)
  std::vector<uint8_t> integrity_region(file_bytes.begin(),
                                        file_bytes.end() - 32);
  ...
  if (computed_hmac.size() != stored_hmac.size() ||
      CRYPTO_memcmp(computed_hmac.data(), stored_hmac.data(),
    ...
    std::cout << "  ╚══════════════════════════════════════════════════════╝\n";
    std::cout
        << "  [✗] Decryption ABORTED — data integrity cannot be guaranteed.\n";
    return;
  }
  ...
  if (dot_pos != std::string::npos)
    out_path =
        out_path.substr(0, dot_pos) + "_decrypted" + out_path.substr(dot_pos);
  else
    out_path += "_decrypted";
  ...
  std::cout << "  ╚══════════════════════════════════════════════════════╝\n";
  std::cout
      << "  [✓] Round-trip: Zstd→SM4-CTR→HMAC → HMAC→SM4-CTR→Zstd verified.\n";
}
